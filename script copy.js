...
async function endGame(lost = false) {
    ...
    restartButton.classList.add('hidden'); // Oculta o botão de reiniciar inicialmente

    if (lost) {
        endGameMessageElement.textContent = 'Você errou e perdeu!';
        rankingMessageElement.innerHTML = `Você não entrou no ranking. <br><br> Últimos ganhadores:`;
        showRanking();
    } else {
        endGameMessageElement.textContent = 'Parabéns, você completou o quiz!';
        rankingMessageElement.textContent = `Verificando se você está no ranking...`;
        ...
        if (isTopPlayer) {
            ...
            try {
                ...
                webcamElement.onloadedmetadata = () => {
                    ...
                    }, 2000); // 2 segundos para a câmera iniciar e tirar a foto
                };
            } catch (err) {
                console.error("Erro ao acessar a webcam: ", err);
                addToRanking('placeholder.png'); // Adiciona com foto placeholder se a webcam falhar
                showRanking();
            }
        } else {
            showRanking();
...
function showRanking() {
    ...
    topScores.forEach(item => {
        ...
        photo.src = item.photo || 'placeholder.png'; // Placeholder se não tiver foto
